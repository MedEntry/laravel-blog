...
class BinshopsBlogPost extends Model implements SearchResultInterface
{
    ...
    protected array $indexContentColumns = ['post_body', 'short_description', 'meta_desc'];

    protected array $indexTitleColumns = ['title', 'subtitle', 'seo_title'];
    ...
    public function search_result_page_url(): string
    {
        return $this->url();
    }
     ...
     * @return int
     */
    public function has_image(string $size = 'medium')
    {
     ...
     * @return string
     */
    public function image_url(string $size = 'medium')
    {
        $this->check_valid_image_size($size);
